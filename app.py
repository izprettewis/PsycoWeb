...
b_age = st.checkbox('Age')

if b_age:
    w_age = st.slider("Choose age:", 18, 70)

w_gender = st.radio("Pick a gender", ('Male','Female','Else'))
#w_language = st.radio("Pick a language", ('EN','FR','PL','DE','PT'))
w_country = st.radio("Pick a country", ('France','Germany','Poland','Denmark','Spain'))
...
def datamix(gender= w_gender, country=w_country): #UserLanguage=w_language, has been removed for now
    if b_age: 
        ...
        mix = data.loc[data['Dem_gender']==gender].loc[data['Country']==country]
    if len(mix)>0:
